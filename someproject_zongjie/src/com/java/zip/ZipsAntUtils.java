 ...
 * 
 * @author tzh
 * @version 创建时间：2018-04-11 下午14:15:21 类说明：
 */
@SuppressWarnings({"unchecked","rawtypes","resource"})
public final class ZipsAntUtils {
	...
	private static final List List = new ArrayList();
	 ...
	 public static final void zip(String needtozipfilepath, String zipfilepath){
	  try {
	   byte[] b = new byte[512];
	 ...
	 public static boolean zip(String baseDirName, String[] fileNames,
	  ...
	  try {
	   ...
	   if (fileNames[0].equals("*")) {
		   ZipsAntUtils.dirToZip(baseDirPath, baseDir, out);
	   } else {
	    ...
	    if (files[0].isFile()) {
	     ...
	     // CompressUtil.fileToZip(baseDirPath, file, out);
	    	ZipsAntUtils.filesToZip(baseDirPath, files, out);
	    }
	 ...
	 private static void filesToZip(String baseDirPath, File[] files,
	  ...
	  for (int i = 0; i < files.length; i++) {
	   ...
	   if (file.isFile()) {
	    // 调用本类的一个静态方法 压缩一个文件
		   ZipsAntUtils.fileToZip(baseDirPath, file, out);
	   } else {
	    /*
	     * 这是一个文件夹 所以要再次得到它下面的所有的文件 这里是自己调用自己..............递归..........
	     */
		   ZipsAntUtils.dirToZip(baseDirPath, file, out);
	   }
	  }
	 }
	 ...
	 public static void main(String[] temp) {
	  // 压缩
	/*  String baseDirName = "F:\\export\\";
	  String[] fileNames = { "*" };
	  String zipFileName = "E:\\zip\\那是一个忧伤的故事.zip";
	  // 压缩多个指定的文件 到ZIP
	   System.out.println(ZipsAntUtils.zip(baseDirName, fileNames,zipFileName,"GBK"));*/

	 /* //压缩一个文件夹 到ZIP
	  String sourcePath = "c:\\test\\";
	  String zipFilePath = "c:\\中文2.zip";
	  ZipsAntUtils.zip(sourcePath, zipFilePath);*/

	  //解压缩
	  System.out.println(ZipsAntUtils.unZip("H:\\usr\\这是一次测试.zip", "E:\\java 工具\\测试文件夹"));
	 }
